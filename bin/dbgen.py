...
def generate_board(board, database):
    print('Generating for {}... '.format(board), end='', flush=True)
...
def main():
    ...
    for board in database["boards"]:
        generate_board(board, database)

    with open(args.database, 'w') as fout:
        print(json.dumps(database, indent=4), file=fout)
