                 ...
                 chan_t *chin_p)
{
    ...
    while (drv_p != NULL) {
        owi_search(drv_p->owi_p);
        ds18b20_convert(drv_p);
        dev_p = drv_p->owi_p->devices_p;

        for (i = 0; i < drv_p->owi_p->len; i++, dev_p++) {
            ...
            std_fprintf(chout_p,
                        ...
                        ds18b20_get_temperature_str(drv_p, dev_p->id, buf));
        }
 ...
 */
static int ds18b20_read_scratchpad(struct ds18b20_driver_t *drv_p,
                                   struct ds18b20_scratchpad_t *scratchpad_p,
                                   uint8_t *id_p)
{
    uint8_t b;

    owi_reset(drv_p->owi_p);
    b = OWI_MATCH_ROM;
    owi_write(drv_p->owi_p, &b, 8);
    owi_write(drv_p->owi_p, id_p, 64);
    b = READ_SCRATCHPAD;
    owi_write(drv_p->owi_p, &b, 8);
    owi_read(drv_p->owi_p, scratchpad_p, 8 * sizeof(*scratchpad_p));
                            ...
                            int *temp_p)
{
    ...
    ds18b20_read_scratchpad(drv_p, &scratchpad, id_p);
                                  ...
                                  char *buf_p)
{
    ...
    ds18b20_get_temperature(drv_p, id_p, &temp);
    std_sprintf(buf_p,
                FSTR("%d.%d"),
                (temp >> 4),
                (temp & 0xf) * 625);
