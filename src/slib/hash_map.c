...
static struct hash_map_entry_t *entry_alloc(struct hash_map_t *map_p)
{
    struct hash_map_entry_t *entry_p;

    entry_p = map_p->entries_p;

    if (entry_p != NULL) {
        map_p->entries_p = entry_p->next_p;
    }

    return (entry_p);
}

static void entry_free(struct hash_map_t *map_p,
                       struct hash_map_entry_t *entry_p)
{
    entry_p->next_p = map_p->entries_p;
    map_p->entries_p = entry_p;
}
                  ...
                  int (*hash)(long key))
{
    ...
    map_p->buckets_p = buckets_p;
    map_p->buckets_max = buckets_max;
    map_p->hash = hash;
                 ...
                 void *value_p)
{
    ...
    /* Hash key. */
    hash = map_p->hash(key);
    hash %= map_p->buckets_max;
    bucket_p = &map_p->buckets_p[hash];
